...
{
    [TestFixture, Description("Fixture for testing WestStrategy for Robot Cleaner")]
    public class WestStrategyTests
    {
        private readonly BaseCleanerStrategy _strategy;
        private Dictionary<int, HashSet<int>> _items;

        public WestStrategyTests()
        {
            _items = new Dictionary<int, HashSet<int>>();
            _strategy = new WestStrategy(_items); 
        }
